...
| `manuscript/simulations/sign_dw_bounded_noise_robust_grid_figure.md` | M0035 bounded-noise Figure 1 candidate: uses pure mixed higher-cumulant J inversion plus a profiled recovered-covariance feasibility screen with `0 <= nu_i <= 0.5`. | Promising candidate visual: high-noise bounded robust DW contains true `B0` and accepts 0.066 of the full plotted grid. Audit before promoting because the noise-variance upper bound is substantive identifying information. |
...
| `manuscript/simulations/sign_dw_robust_noise_grid_figure.py` | Manuscript-local B-plane grid generator. M0034 added `--robust-mode pure`; M0035 added `--robust-mode bounded`. The old default diagonal-anchor output is superseded for the robust row. | Use bounded mode for the current candidate Figure 1 and pure mode as the fallback comparison; audit before promoting. |
...
| `manuscript/simulations/m28_grid_story_validation.py` | Refreshed M28 validation pass for the superseded M0030 grid pair. | Historical for the robust row after M0034/M0035; rerun after M40 if the bounded screen passes audit. |
| `manuscript/simulations/m29_calibrated_monte_carlo.py` | Refreshed M29 finite-sample pass for the superseded M0030 robust statistic. | Historical for the robust row after M0034/M0035; reuse its reporting metrics when rebuilding the bounded-noise Monte Carlo. |
