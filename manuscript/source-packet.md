...
| `manuscript/simulations/m74_sample_size_mc_500_grid27.md` and `manuscript/simulations/output/m74_sample_size_mc_500_grid27.json` | M74/M75 500-replication sample-size Monte Carlo for Figure 3 only: fixes \(V=\operatorname{diag}(0.2,0.2)\), strong structural non-Gaussianity, the M71 sign screen, candidate-specific pointwise weights, and an intermediate `27/7/5` grid while varying `T=500,1000,2000`. | Active detailed sample-size evidence after M75; use `draft.md#55-detailed-sample-size-monte-carlo` for the audit-friendly DGP, set computation, coverage/size, and warning interpretation. Pointwise chi-square cutoffs remain diagnostic, with final projected critical values still assigned to M65. |
