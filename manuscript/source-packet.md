...
| `manuscript/simulations/m74_sample_size_mc_500_grid27.md` and `manuscript/simulations/output/m74_sample_size_mc_500_grid27.json` | M74/M75/M76 500-replication sample-size Monte Carlo for Figure 3 only: fixes \(V=\operatorname{diag}(0.2,0.2)\), strong structural non-Gaussianity, the M71 sign screen, candidate-specific pointwise weights, and an intermediate `27/7/5` grid while varying `T=500,1000,2000`. | Active detailed sample-size evidence after M76; use `draft.md#55-detailed-sample-size-monte-carlo` for the audit-friendly DGP, set computation, coverage/size, warning interpretation, and the Sign/DW/nrDW naming convention. Pointwise chi-square cutoffs remain diagnostic, with final projected critical values still assigned to M65. |
| `manuscript/simulations/m77_clean_iid_mc_efficient_weight.md` and `manuscript/simulations/output/m77_clean_iid_mc_efficient_weight.json` | M77 cleaned iid pointwise size audit: removes sample standardization and demeaning, uses population-normalized iid \(\chi^2_5\) structural shocks and iid Gaussian residual noise, and computes analytic iid efficient weights \(W=(E[f_tf_t'])^{-1}\) by polynomial expansion rather than auxiliary simulation. | Active supplement to M74: truth-at-\(B_0\) only, no projection shares or empty-set rates. It shows nrDW pointwise truth inclusion of 0.884, 0.896, and 0.900 for \(T=500,1000,2000\), indicating that the M74 nrDW undercoverage was mainly an implementation/weighting calibration issue. |
