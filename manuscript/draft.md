...
Unless the off-diagonal elements of \(B_0^{-1}VB_0^{-1'}\) are zero, the shocks
...
\comment{ also delete this definition. lets only have a set S_0. without noise we have B_0 element S_0 and with noise in general B_0 not in S_0.}
...
\comment{i think we dont need the following paragraph elaborating on  \(B_0D^2B_0'\). we already wrote that B_0 is not in S_0 unless the off-diagonal elements of \(B_0^{-1}VB_0^{-1'}\) are zero. so lets leave it like that. but briefly mentioning that the noise can even flip a sign is good.}
...
chart before higher moments are used. \comment{we should create a new figure which contains only the first row of figure 1 and show it here. however, lets discuss and show this figure at the end of the subsection. i first want to introduce the j statistic which we invert.}
...
candidate \(B\), let \commet{add \widehat variance moments!! T^{-1}\sum_{t=1}^T e_{1t}^2(B)-1 and  T^{-1}\sum_{t=1}^T e_{1t}^2(B)-2}
...
\comment{delete the following paragraph on rotations}
...
If the structural shocks are non-Gaussian and independent. DW propose to refine the set \(S_0\) by avoiding  \(B\) matrices which lead to uncorrelated, but dependent shocks.
...
\comment{lets clean up this paragraph. no mention of m49. just explain that dw measure dependency by third and fourth order co-moments and then introduce g_{DW,1}(B). }
...
sign-admissible candidates whose estimated moment vector is small: \comment{this is not correct because you are missing the variance and covariance conditions from m_2! also mention efficient weighting, distribution of J, similar to what we write after the inverted set in section 2. }
...
\comment{do not use the diag one normalization! instead, we dont need the s_1 s_2 because they are 1 by normalization. also, do you use S=E[u_t u_t]? if yes, we always use \Sigma_u=E[u_t u_t] and we call the elements of \Sigma_u \sigma_{ij}! the whole folowing part on how to allow for correlated shocks based on the information noise ration is confusing to me. Can we do it like this: In general u=B_0 epsilon + eta and \Sigma_u=E[u_t u_t] = B_0 B_0' + V, is this correct? if yes, we can say that for every B, lets introduce the restriction \Sigma_u - BB' > diag(rho). would this only allow B which lead to a information noise smaller than rho? in the end, we certainly have to replace the moment conditions m_2 from section on enforcing uncorrelated unit variance e with new noise robust second order moment conditions, enforcing unit variance (?) and correlated shocks up to the information noise ratio. i really want a clear derivative of how this works right here!}
...
\comment{ now the notation gets even more confusing! we already talked about that S should be \Sigma_u, correct? but now you use \Omega for the variance covariance of zeta=B^-1 eta? thats also terrible bwecause in svar, \Omega is typuically the variance covariance matrix of structural shocks epsilon. so we can define E[e(B) e(B)']=}
