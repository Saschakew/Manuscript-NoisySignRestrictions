...
\comment{ now the notation gets even more confusing! we already talked about that S should be \Sigma_u, correct? but now you use \Omega for the variance covariance of zeta=B^-1 eta? thats also terrible bwecause in svar, \Omega is typuically the variance covariance matrix of structural shocks epsilon. is \Sigma_}
