...
| File | Source | Status |
|---|---|---|
| `fig_sign_dw_unit_variance_noise_grid.png` | `../simulations/sign_dw_unit_variance_noise_grid_figure.py` | Active M67 Figure 1 diagnostic: projects accepted unit-variance candidates onto `(B12,B21)`, profiles `B11`, `B22`, and `lambda`, and uses the M66 bound `lambda_i=nu_i/(BB')_ii in [0,rho]`. |
...
| `fig_sign_dw_robust_nongaussianity_grid.png` | `../simulations/sign_dw_robust_nongaussianity_grid_figure.py` | Historical Figure 2 candidate after M64/M66: fixes residual noise, weakens structural non-Gaussianity, and still uses the pre-M64 variance-ratio robust row. M65 must rebuild it. |
