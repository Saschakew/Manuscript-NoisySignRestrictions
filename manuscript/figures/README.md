...
| File | Source | Status |
|---|---|---|
| `fig_sign_dw_robust_nongaussianity_grid.png` | `../simulations/sign_dw_robust_nongaussianity_grid_figure.py` | M0020 corrected companion candidate: fixes residual noise and varies structural-shock non-Gaussianity across columns; all rows invert pointwise 10 percent J tests, and the robust-DW row uses only mixed higher cumulants. |
| `fig_sign_dw_robust_noise_grid.png` | `../simulations/sign_dw_robust_noise_grid_figure.py` | M0020 corrected candidate: reproduces the KnowledgeVault 2-by-3 sign/DW noise grid with a third robust-DW row; all rows invert pointwise 10 percent J tests, and the high-noise column shows standard DW rejecting true `B0` while robust DW contains it. Preferred over the M0016 three-panel candidate for the user's requested visual. |
