...
E\{z_1(B)z_2(B)^3\}-3s_{22}(B)s_{12}(B) where the s depent on the B and the variance of the noise, correct? so instead of computing s as a sample average, cant we just tread the variances of the noise as a parameter we estimate and for a given noise variance parameter and given B parameter, we can compute the s_{ab} terms in the moment conditions? then we can write our moment conditions as standard gmm moment conditions.
