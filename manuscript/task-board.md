...
| M26 | doing | 1 | figure | Design the intuitive noisy sign-set figure. | M0020 corrected the requested 3-by-3 sign/DW/robust-DW B-plane noise grid and its non-Gaussianity companion so all rows invert pointwise 10 percent J tests; robust rows use only higher cumulants and no second moments. Decide after M28/M29 whether these become the main evidence figures. |
...
| M28 | todo | 1 | simulation | Build population-grid verification for standard DW and robust DW. | Use the M30 audit findings and M0020 candidate figures: verify structural-rescaling exceptions, generic noisy pseudo-zeros, robust-DW truth inclusion, finite-stack aliases, weak-moment widening, and whether the pointwise J-test critical values are defensible. |
