...
| M26 | done | 1 | figure | Design the intuitive noisy sign-set figure. | M0020 produced the selected visual spine: the residual-noise grid and the non-Gaussianity grid. Keep them as the main story figures unless M28/M29 invalidate them. |
| M28 | todo | 1 | simulation | Validate the M0020 grid-pair story. | First next task. Recompute the grid story with population moments, repeated seeds, grid-boundary sensitivity, true-`B0` J diagnostics, and critical-value sensitivity. Verify standard-DW truth rejection under noise, robust-DW truth inclusion, and weak-moment widening. |
| M27 | todo | 1 | method | Formalize the robust DW accepted set and comparison diagnostic. | After the validation target is clear, define standard DW set, robust DW set, overlap/divergence metric, critical values, and interpretation boundaries using the M0020 figure language. |
| M29 | todo | 1 | simulation | Build finite-sample Monte Carlo evidence around the figure pair. | Report coverage, accepted-set share or width, empty-set frequency, overlap frequency, and divergence warnings across no-noise, moderate-noise, high-noise, weak-moment, Gaussian-shock, and misspecified-noise cases. |
...
| M31 | todo | 2 | writing | Draft the figure-led section skeleton with source trails. | After M28 validation, rewrite the introduction and evidence section around the two-grid story: noise grid first, non-Gaussianity grid second. |
