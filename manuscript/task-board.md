...
| M06 | done | 1 | adversarial-review | Audit the cumulant derivation before using it. | Completed M06 audit in `manuscript/review-log.md` and `manuscript/derivations/bivariate-cumulant-map.md`; no coefficient/index errors found, and classification wording was corrected so clean equations are not overstated as identifying restrictions after nuisance profiling. |
| M07 | done | 1 | method | Prove what the BR analogy does and does not justify. | Created `manuscript/derivations/br-applicability.md`; it shows the direct all-kurtotic BR rank condition fails because independent bivariate errors give `J=1<K=2`, the skewness route also does not cover the two-factor case, and the manuscript object is a BR-style profiled inversion rather than quasi-JADE. |
| M22 | done | 1 | method | Derive a DW-like Gaussian-noise higher-moment route. | Created `manuscript/derivations/dw-noise-robust-moments.md`; it distinguishes the invalid no-noise covariance-whitened DW approach from a normalized sign-plus-higher-cumulant system, writes fourth cumulants as raw-moment equations with nuisance covariance products, and derives local bivariate rank conditions under Gaussian residual noise. |
| M23 | todo | 1 | adversarial-review | Audit the DW-like Gaussian-noise route before changing the paper structure. | Check cumulant-to-moment algebra, whether second moments are only nuisance ingredients, the normalization/scale loss, local-rank conditions, global aliases, finite-sample bias language, and whether the route should replace or sit beside the BR-style observed-residual route. |
