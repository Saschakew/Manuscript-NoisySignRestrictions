...
and an M30 audit of that triage. The evidence queue is now centered on
...
| M5. Evidence package | doing | M35 early Monte Carlo triage and M30 audit are complete; population-grid checks must come before polished figures or final Monte Carlo tables. |
...
6. Diagnostic interpretation review: verify that DW-versus-robust-DW
   ...
   error.
7. Evidence design: use the M35/M30 result as a cautionary gate; run
   population grids before expanding simulations to no-noise agreement, noisy
